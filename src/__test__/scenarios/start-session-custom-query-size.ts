...
export function startSessionCustomQuerySize({
  books,
  querySize,
}: {
  books?: KindleBookData[];
  querySize: number;
}) {
  return {
    startSession: {
      request: {
        ...
        body: {
          ...defaultRequestBody,
          requestUrl: `https://read.amazon.com/kindle-library/search?query=&libraryType=BOOKS&sortType=acquisition_desc&querySize=${querySize}`,
        },
      },
      response: {
        headers: {},
